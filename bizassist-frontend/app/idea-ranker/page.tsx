    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div className='text-center'>
          <Loader2 className={`w-12 h-12 animate-spin mx-auto mb-4 ${isDark ? 'text-blue-500' : 'text-emerald-500'}`} />
          <p className={`text-lg ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>Analyzing your business idea...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div className='text-center max-w-md'>
          ...
          <p className={`text-lg mb-4 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{error}</p>
          <button
            onClick={() => (window.location.href = '/')}
            className={`px-6 py-3 rounded-lg font-semibold ${isDark ? 'bg-blue-600 hover:bg-blue-700' : 'bg-emerald-600 hover:bg-emerald-700'} text-white`}
          >
            Go Back Home
          </button>
        </div>
      ...
      <main className='max-w-7xl mx-auto px-6 lg:px-12 py-12'>
        <div className='mb-12'>
          ...
          <p className={`text-lg ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            Comprehensive analysis of your idea's potential in the Bangladesh market
          </p>
        </div>
          ...
          <div className='flex flex-col gap-6'>
            <div
              className={`p-8 rounded-2xl border ${
                isDark ? 'bg-gradient-to-br from-blue-600 to-blue-700 border-blue-500'
                      : 'bg-gradient-to-br from-emerald-600 to-emerald-700 border-emerald-500'
              } text-white shadow-xl`}
            >
              <p className='text-lg font-semibold mb-2 opacity-90'>Pitch Readiness Index</p>
              <p className='text-sm font-bold mb-4'>{rankerData?.readinessLabel || 'Analyzing...'}</p>
              <div className='flex items-end gap-2'>
                <p className='text-7xl font-black'>{rankerData?.overallScore || 0}</p>
                <span className='text-4xl opacity-80 mb-2'>/100</span>
              </div>
            </div>

            <div
              className={`p-8 rounded-2xl border ${isDark ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200'} shadow-xl`}
            >
                ...
                <ul className='space-y-4'>
                  {rankerData.nextSteps.map((step, idx) => (
                    <li key={idx} className='flex items-start gap-3'>
                      ...
                      <p className={`text-sm leading-relaxed ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{step.text}</p>
                    </li>
                  ))}
                </ul>
              ) : (
        ...
        {competitors && competitors.competitors && competitors.competitors.length > 0 && (
          <div
            className={`rounded-2xl border ${isDark ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200'} shadow-xl overflow-hidden mb-12`}
          >
            <div className={`p-8 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
              <h2 className='text-2xl font-bold mb-2'>Top Competitors</h2>
              <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Key players in your market space</p>
            </div>
            <div className='p-8'>
              {isLoadingCompetitors ? (
                <div className='flex items-center justify-center py-12'>
                  <Loader2 className={`w-8 h-8 animate-spin ${isDark ? 'text-blue-500' : 'text-emerald-500'}`} />
                </div>
              ) : (
                <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
                  {competitors.competitors.map((competitor, idx) => (
                    <CompetitorCard key={idx} competitor={competitor} isDark={isDark} />
                  ))}
                </div>
              )}
            </div>
        ...
        <div className={`rounded-2xl border ${isDark ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200'} shadow-xl overflow-hidden`}>
          <div className={`p-8 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
            ...
            {rankerData &&
              Object.entries(rankerData.scores).map(([key, value], idx) => {
                ...
                return (
                  <div
                    key={key}
                    className={`flex flex-col gap-3 border-t py-6 ${idx % 2 === 0 ? 'md:pr-4' : 'md:pl-4'} ${
                      isDark ? 'border-gray-700' : 'border-gray-200'
                    }`}
                  >
                    <div className='flex items-center justify-between'>
                      <p className={`text-sm font-bold uppercase tracking-wide ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                        {labels[key] || key}
                      </p>
                      <span className={`px-3 py-1 rounded-full text-sm font-bold ${
                        isDark ? 'bg-blue-600/20 text-blue-400' : 'bg-emerald-100 text-emerald-700'
                      }`}>
                        {scoreValue.score}/100
                      </span>
                    </div>
                    <p className={`text-sm leading-relaxed ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{scoreValue.justification}</p>
                  </div>
                )
              })}
          </div>
