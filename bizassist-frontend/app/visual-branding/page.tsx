...
const VisualBrandingScreen = () => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        const storedBusinessTitle = sessionStorage.getItem('bizassist-business-title');
        ...
        if (storedSummary) {
          const nameToUse = storedBusinessTitle || businessName || 'Your Business';
  ...
  const generatePalettes = async (logoColors: string[] = [], summaryText?: string, businessNameText?: string) => {
    // Use provided parameters or fall back to state
    const summaryToUse = summaryText || summary;
    const businessNameToUse = businessNameText || businessName || 'Your Business';
    ...
    try {
      const apiUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/generate-palettes`;
  ...
  const currentPalette = palettes[selectedPalette] || palettes[0] || {
    name: 'Default',
    colors: ['#5B5FEF', '#10B981', '#F59E0B', '#F9FAFB']
  };
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div className="text-center">
          <Loader2 className={`w-12 h-12 animate-spin mx-auto mb-4 ${isDark ? 'text-blue-500' : 'text-indigo-600'}`} />
          <p className={isDark ? 'text-gray-300' : 'text-gray-600'}>Generating your visual identity...</p>
        </div>
      </div>
    );
  }
      ...
      <main className="max-w-7xl mx-auto px-6 lg:px-10 py-10">
        ...
        <div className="flex flex-wrap justify-between items-start gap-4 mb-10">
          <div>
            ...
            {businessName && (
              <p className={`text-2xl font-bold mb-2 ${isDark ? 'text-blue-400' : 'text-indigo-600'}`}>
                {businessName}
              </p>
            )}
            <p className={`text-base max-w-2xl ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              Select a logo and color palette to see them applied to your slides. The color palettes are AI-generated based on your business.
            </p>
          </div>
            ...
            <button
              onClick={() => {
                ...
                try {
                  ...
                  sessionStorage.setItem('bizassist-branding', JSON.stringify(brandingData));
                } catch (error) {
                  console.error('Error saving branding:', error);
                }
                window.location.href = '/pitch-improvement';
              }}
